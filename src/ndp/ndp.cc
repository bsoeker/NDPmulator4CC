...
NDP::NDP(const NDPParams &params)
    : ClockedObject(params), cpuPort(params.name + ".cpu_side", this),
      memPort(params.name + ".mem_side", this),
      dmaPort(params.name + ".dma_port", this), ndpCtrl(params.ndp_ctrl),
      ndpData(params.ndp_data), maxRSze(params.max_rsze),
...
AddrRangeList NDP::CPUSidePort::getAddrRanges() const {
  return owner->getAddrRanges();
}

void NDP::CPUSidePort::recvFunctional(PacketPtr pkt) {
  owner->memPort.sendFunctional(pkt);
}

bool NDP::CPUSidePort::recvTimingReq(PacketPtr pkt) {
...
void NDP::CPUSidePort::recvRespRetry() {
  panic("CPUSidePort::recvRespRetry not implemented!\n");
}

bool NDP::handleRequest(PacketPtr pkt) {
  ...
  schedule(new EventFunctionWrapper(
               [this, pkt] {
                 panic_if(!cpuPort.sendTimingResp(pkt),
                          "CPU did not accept response from NDP device!\n");
               },
               name() + ".sendTimingRespEvent", true),
...
void NDP::sendData() {
  ...
  DPRINTF(NDPMem, "NDP device %s %lu bytes %s %p\n",
          pkt->isWrite() ? "writing" : "reading", pkt->getSize(),
          pkt->isWrite() ? "to" : "from", pkt->getAddr());
  ...
  if (dmaPort.sendTimingReq(pkt)) {
    ...
    if (!pendingReqPackets.empty()) {
      schedule(new EventFunctionWrapper([this] { sendData(); },
...
void NDP::accessMemory(Addr addr, size_t size, bool write, uint8_t *data) {
  // Create new burst request
  BurstRequest *newRequest =
      new BurstRequest(AddrRange(addr, addr + size), data, write);

  uint64_t nRequests = 0;
  for (Addr saddr = addr; saddr < addr + size; saddr += maxRSze, ++nRequests) {
    // Calculate size of new request
    size_t ssize =
        nRequests * maxRSze + maxRSze > size ? size % maxRSze : maxRSze;
...
bool NDP::memCallback(PacketPtr pkt) {
  DPRINTF(NDPMem, "NDP device received %s response (%p, %lu bytes)\n",
    ...
    if ((*it)->isWrite() == pkt->isWrite()) {
      ...
      if (dataPtr) {
        ...
        if ((*it)->requestComplete()) {
          DPRINTF(NDPMem, "Completed %s request (%p, %lu bytes)\n",
...
bool NDP::MemSidePort::recvTimingResp(PacketPtr pkt) {
  // Request was made by the CPU through mem_side
  if (name().find("mem_side") != std::string::npos) {
    panic_if(!owner->cpuPort.sendTimingResp(pkt),
...
void NDP::MemSidePort::recvReqRetry() {
  // Request was made by the CPU through mem_side
  if (name().find("mem_side") != std::string::npos)
...
void NDP::MemSidePort::recvRangeChange() { owner->sendRangeChange(); }

AddrRangeList NDP::getAddrRanges() const { return memPort.getAddrRanges(); }

void NDP::sendRangeChange() const { cpuPort.sendRangeChange(); }

uint8_t *NDP::BurstRequest::popPendingSubRequest(AddrRange addrRange) {
  for (auto it = pendingSubRequests.begin(); it != pendingSubRequests.end();
       ++it) {
    // Subrequest matches addrRange
    if ((*it)->getAddrRange() == addrRange) {
      uint8_t *rDataPtr = (*it)->getDataPtr(); // Retrieve original pointer
